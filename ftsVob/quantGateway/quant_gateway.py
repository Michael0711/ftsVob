...
class VtGateway(object):
    ...
    def onTick(self, tick):
        """市场行情推送"""
        # 通用事件
        # event = Event(event_type=EVENT_TICKET)
        # event.data = tick
        # self.eventEngine.put(event)

        # 用RingBuffer将行情数据缓存以便于将来可以合并
        if tick.symbol in self.tickdata:
            self.tickdata[tick.symbol].append(tick)
        else:
            self.tickdata[tick.symbol] = RingBuffer(size_max = 900)
            self.tickdata[tick.symbol].append(tick)
